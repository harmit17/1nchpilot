...
export default function PortfolioAnalytics({ onClose }: PortfolioAnalyticsProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      console.log('📊 Portfolio report received:', data.report);
    } catch (err: any) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      const summaryText = `This comprehensive analysis covers wallet ${walletShort} on Ethereum network as of ${reportDate}. Our AI-powered system analyzed ${report.metrics.tokenCount} tokens with a total value of $${report.metrics.totalValue.toLocaleString()}, providing actionable insights for portfolio optimization.`;
